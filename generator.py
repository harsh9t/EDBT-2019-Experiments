...
def create_warm_query(query, graph, dataset, path, index = None):
	indexing_syn = ""
	if index is not None:
		if graph == "sparksee":
			for each in index:
...
def create_warm_query_for_tinker3(query, dataset, path, index = None):
	indexing_syn = ""
	if index is not None:
		for each in index:
			indexing_syn += 'x.createIndex("%s", Vertex.class)\n' %(each);
...
def create_queries(dataset_location, index = None, warm = True):
	...
	for each in ['neo4j', 'sparksee', 'tinkergraph']:
		...
		for query in all_queries:
			if warm:
				create_warm_query(open(query).read(), each, dataset_location.strip().split("/")[-1], dataset_location + '/%s/%s' % (each, get_file_name(query)), index)
			else:
				create_query(open(query).read(), each, dataset_location.strip().split("/")[-1], dataset_location + '/%s/%s' % (each, get_file_name(query)), index)

def create_queries_3(dataset_location, index = None, warm = True):
	try:
	...
	for query in tinker3_queries:
		if warm:
			create_warm_query_for_tinker3(open(query).read(), dataset_location.strip().split("/")[-1], dataset_location + '/tinker3/%s' % (get_file_name(query)), index);
		else:
...
if __name__ == "__main__":
	create_queries(os.getcwd() + "/nwq", index = ['name', 'customerId', 'unitPrice', 'unitsInStock', 'unitsOnOrder'], warm = True)
	create_queries(os.getcwd() + "/bsbmq", index = ["productID", "label_n", "type", "productTypeID", "reviewerID"], warm = True)
	create_queries_3(os.getcwd() + "/nwq", index = ['name', 'customerId', 'unitPrice', 'unitsInStock', 'unitsOnOrder'], warm = True)
	create_queries_3(os.getcwd() + "/bsbmq", index = ["productID", "label_n", "type", "productTypeID", "reviewerID"], warm = True)	
